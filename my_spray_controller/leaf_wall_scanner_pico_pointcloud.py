...
class VerticalLidarNode(Node):
    ...
    def __init__(self):
        ...
        self.scan_subscriber = self.create_subscription(
            PointCloud2, '/picoScan_25420273/cloud/all_points', self.scan_callback, 10
        )
    ...
    def scan_callback(self, msg: PointCloud2):
        """Process incoming PointCloud2 from SICK PicoScan and accumulate into 3D point cloud."""
        current_time = self.get_clock().now()
        
        # Store the frame_id from the point cloud for consistency
        ...
        if self.last_scan_time is not None and self.current_speed != 0.0:
            ...
            if True:
                ...
                self.get_logger().info(f"Scan rate: {scan_rate:.1f} Hz, Travelled distance: {self.distance_traveled:.2f}m")
                self.logger_count = 0
        ...
        for point in pc2.read_points(msg, field_names=["x", "y", "z"], skip_nans=True):
            # Transform coordinates: Pico (x,y,z) -> Your system (x,y,z)
            # Pico: x=up, y=left, z=forward -> Your: x=forward, y=left, z=up
            your_x = self.distance_traveled  # forward (driving direction)
            your_y = -point[1]              # left (negative pico y)
            your_z = point[0] + self.lidar_height  # up (pico x + height offset)
            
            points_list.append([your_x, your_y, your_z])
        
        if len(points_list) == 0:
            self.get_logger().info(f"-----------No valid points-------: Len_Points: {len(points_list)}")
            return
